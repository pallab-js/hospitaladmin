...
const ADMISSION_DETAIL_QUERY: &str = r#"SELECT a.*,
...
pub async fn create_admission(
    ...
    tx.commit()
        .await
        .map_err(|_| "Failed to commit admission".to_string())?;
    ...
    sqlx::query_as::<_, AdmissionWithDetails>(
        format!("{} WHERE a.id = ?", ADMISSION_DETAIL_QUERY).as_str(),
    )
    .bind(&id)
    .fetch_optional(pool)
    .await
...
pub async fn discharge_patient(request: DischargeRequest) -> Result<(), String> {
    ...
    tx.commit()
        .await
        .map_err(|_| "Failed to commit discharge".to_string())?;
