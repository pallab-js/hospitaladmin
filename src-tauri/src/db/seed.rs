...
async fn seed_staff(pool: &SqlitePool) -> Result<(), Box<dyn std::error::Error>> {
    let staff_data = vec![
        ("Sarah", "Johnson", "doctor", "GM", "MBBS, MD", "General Medicine"),
        ("Michael", "Chen", "doctor", "CARD", "MBBS, DM", "Cardiology"),
        ("Emily", "Williams", "doctor", "ORTH", "MBBS, MS", "Orthopedics"),
        ("David", "Brown", "doctor", "PED", "MBBS, MD", "Pediatrics"),
        ("Lisa", "Davis", "doctor", "EMER", "MBBS, Diploma in Emergency Medicine", "Emergency"),
        ...
        ("John", "Martinez", "lab_tech", "PATH", "B.Sc Medical Lab Tech", ""),
    ];
...
async fn seed_users(pool: &SqlitePool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let staff = sqlx::query("SELECT id, first_name, last_name FROM staff WHERE role != 'doctor' LIMIT 3")
        .fetch_all(pool)
        .await?;
...
async fn seed_wards_and_beds(pool: &SqlitePool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, floor, ward_type, bed_count) in wards {
        ...
        sqlx::query("INSERT INTO wards (id, name, floor, ward_type, is_active) VALUES (?, ?, ?, ?, 1)")
            .bind(&ward_id)
            .bind(name)
            .bind(floor)
            .bind(ward_type)
...
async fn seed_medications(pool: &SqlitePool) -> Result<(), Box<dyn std::error::Error>> {
    let medications = vec![
        ("Paracetamol 500mg", "Paracetamol", "Analgesic", "Tablet", "500mg", "Cipla", 2.0),
        ("Amoxicillin 250mg", "Amoxicillin", "Antibiotic", "Capsule", "250mg", "Sun Pharma", 8.0),
        ("Metformin 500mg", "Metformin", "Antidiabetic", "Tablet", "500mg", "USV", 5.0),
        ("Omeprazole 20mg", "Omeprazole", "Antacid", "Capsule", "20mg", "Dr. Reddy's", 6.0),
        ("Cetirizine 10mg", "Cetirizine", "Antihistamine", "Tablet", "10mg", "Cipla", 3.0),
        ("Amlodipine 5mg", "Amlodipine", "Antihypertensive", "Tablet", "5mg", "Pfizer", 7.0),
        ("Azithromycin 500mg", "Azithromycin", "Antibiotic", "Tablet", "500mg", "Zydus", 12.0),
        ("Pantoprazole 40mg", "Pantoprazole", "Antacid", "Tablet", "40mg", "Alkem", 8.0),
        ("Diclofenac 50mg", "Diclofenac", "NSAID", "Tablet", "50mg", "Novartis", 4.0),
        ("Salbutamol Inhaler", "Salbutamol", "Bronchodilator", "Inhaler", "100mcg", "Cipla", 150.0),
    ];
...
async fn seed_lab_tests(pool: &SqlitePool) -> Result<(), Box<dyn std::error::Error>> {
    let tests = vec![
        ("Complete Blood Count", "CBC", "Pathology", "4.0-5.5 million/uL", "million/uL", 150.0),
        ("Blood Sugar Fasting", "BSF", "Pathology", "70-100 mg/dL", "mg/dL", 80.0),
        ("Blood Sugar Post Prandial", "BSPP", "Pathology", "80-140 mg/dL", "mg/dL", 80.0),
        ("Lipid Profile", "LP", "Pathology", "Desirable <200 mg/dL", "mg/dL", 300.0),
        ("Liver Function Test", "LFT", "Pathology", "AST 10-40 U/L", "U/L", 400.0),
        ("Kidney Function Test", "KFT", "Pathology", "Creatinine 0.6-1.2 mg/dL", "mg/dL", 350.0),
        ("Thyroid Profile", "TFT", "Pathology", "TSH 0.4-4.0 mIU/L", "mIU/L", 500.0),
        ("Urine Routine", "UR", "Pathology", "Normal", "", 100.0),
        ("ECG", "ECG", "Cardiology", "Normal Sinus Rhythm", "", 200.0),
        ("X-Ray Chest", "XRC", "Radiology", "No active lesion", "", 400.0),
    ];
...
async fn seed_sample_patients(pool: &SqlitePool) -> Result<(), Box<dyn std::error::Error>> {
    let patients = vec![
        ("Rajesh", "Kumar", "1985-03-15", "male", "B+", "9876543210", "123 Main St"),
        ("Priya", "Sharma", "1990-07-22", "female", "O+", "9876543211", "456 Oak Ave"),
        ("Amit", "Patel", "1978-11-08", "male", "A+", "9876543212", "789 Pine Rd"),
        ("Sneha", "Reddy", "1995-01-30", "female", "AB+", "9876543213", "321 Elm St"),
        ("Vikram", "Singh", "1982-06-18", "male", "O-", "9876543214", "654 Maple Dr"),
    ];
